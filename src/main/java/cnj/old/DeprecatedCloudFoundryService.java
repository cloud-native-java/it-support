...
/*
	* don't use this! It's only here as we pick apart it's code and refactor..
	*/
@Deprecated
class DeprecatedCloudFoundryService {

		private final CloudFoundryOperations cf;
		private Sanitizer sanitizer = new Sanitizer();
		private Log log = LogFactory.getLog(getClass());

		public DeprecatedCloudFoundryService(CloudFoundryOperations cf) {
				this.cf = cf;
		}
		...
		public void createServiceBroker(String serviceBrokerName, String url,
																																		String user, String pw, boolean spaceScoped) {
		...
		public void pushApplicationUsingManifest(File jarFile,
																																											ApplicationManifest applicationManifest, Map<String, String> envs,
																																											boolean start) {

				log.debug("pushing application " + jarFile.getAbsolutePath()
					+ " using applicationManifest file " + applicationManifest.toString());
				PushApplicationRequest request = fromApplicationManifest(jarFile,
					applicationManifest);
